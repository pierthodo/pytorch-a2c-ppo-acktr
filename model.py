...
class Policy(nn.Module):
    def __init__(self, obs_shape, action_space,num_processes,num_steps,N_backprop,sub_reward,algo, base_kwargs=None):
    ...
    def evaluate_actions(self, inputs, rnn_hxs, masks, action,indices,rewards,prev_value_list):
        ...
        if self.algo == "a2c":
            ...
            for i in range(inputs.size()[0]):
                value, actor_features, rnn_hxs, beta_v = self.base(inputs[i, :, :], rnn_hxs, masks[i, :, :])

                eval_prev_value = masks[i, :, :] * prev_value + (1 - masks[i, :, :]) * value


                value = beta_v * value + (1 - beta_v) * eval_prev_value

                beta_list.append(beta_v)
                value_list.append(value)
                if self.sub_reward:
                    value  = value - rewards[i,:,:]
                dist = self.dist(actor_features)
            ...
            value_mixed = torch.stack(value_list)
            beta_v = torch.stack(beta_list)
